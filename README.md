...
| `.UIManager.js`              | Application UI and elements.                                 |
| `.preload.js`                | Give the Renderer process controlled access to some Node API.|
| `.authConfig.js`             | Configuration objects to be passed to MSAL instanc.          |
...
1. In the **Redirect URIs** list, under **Suggested Redirect URIs for (mobile, desktop)** be sure to add `msal{Your_Application/Client_Id}}://auth`.
1. Select **Configure**.
...
1. Open the [.authConfig.js](./App/authConfig.js) file and provide the required configuration values.
    ...
    1. Replace the string `Enter_the_Redirect_Uri_Here` with the default redirect uri generated by portal `msal{Your_Application/Client_Id}://auth`
